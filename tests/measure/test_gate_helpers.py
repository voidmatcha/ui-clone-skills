...
def test_gate_hidden_children_status_pass_passes(tmp_path: Path) -> None:
    ...
    (ref / "hidden-children.json").write_text(
        json.dumps(
            {
                "schemaVersion": 1,
                "status": "pass",
...
def test_gate_runtime_dom_parity_status_fail_fails(tmp_path: Path) -> None:
    ...
    (ref / "runtime-dom-parity.json").write_text(
        json.dumps(
            {
                "schemaVersion": 1,
                "status": "fail",
...
def test_gate_blank_viewport_status_fail_fails(tmp_path: Path) -> None:
    """status:fail on blank-viewport artifact → gate fails."""
    from ui_clone.gate import Gate

    ref = tmp_path / "ref"
    ref.mkdir()
    _baseline_post_implement_inputs(ref)
    _make_verification_plan(ref, "blank-viewport", "blank-viewport.json")
    ...
    results = Gate(ref).gate_post_implement()
    failures = [r for r in results if r.status == "fail"]
    assert any("blank-viewport" in r.label for r in failures), results
...
def test_gate_runtime_dom_parity_missing_status_fails(tmp_path: Path) -> None:
    ...
    failures = [r for r in results if r.status == "fail"]
    assert any(
        "runtime-dom-parity" in r.label and "status` field" in (r.message or "") for r in failures
    ), failures
...
def test_gate_rapid_phase_does_not_downgrade_anti_cheat(tmp_path: Path) -> None:
    ...
    (ref / "ref-screenshot-asset.json").write_text(
        json.dumps(
            {
                "schemaVersion": 1,
                "status": "fail",
    ...
    fails = [r for r in results if r.status == "fail" and "ref-screenshot-asset" in r.label]
...
def test_gate_transition_health_silent_when_sections_have_zero_passes(
    ...
    assert not any("transitions/result.txt visual health" == r.label for r in failures), failures
