...
| **Scroll-scrub / sticky-pin section rendered STATIC → height collapses → accumulated vertical offset breaks alignment for ALL lower sections** | The ref scroll-scrub section has NO inline/CSS height — its height comes from a **pin + runway** (sticky inner pinned inside a tall outer track that provides scroll distance). Rendered static (content + no runway), it collapses to content height, so it is hundreds of px SHORTER than ref. Each collapsed section shifts every lower section UP; the offsets accumulate (real failure: impl ~2120px shorter than ref by `eatReal`). section-compare scrolls to ref scroll positions and crops each section, so EVERY lower section then fails on vertical alignment **even when its own content is correct** — section-compare scored 1/13 from this alone. Compounded by content frozen at init state (read-along text, per-char disintegration, food-pyramid opacity/scale, horizontal marquee `translateX`, doc-card slide-up stack stay at start values → AE 28k–375k vs ~500–2000 threshold). **Detection:** classified scroll-scrub in `transition-coverage.json`, non-trivial child start states in `animation-init-styles.json`, and/or a `scrollYProgress`/`ScrollTrigger` driver in `bundles/*.js`; confirm by measuring impl vs ref section heights. | **Height MUST come from the runway, not the content.** Render an outer track div with `height` = the measured ref section height, and a `position: sticky; top: 0; height: 100vh` inner that pins inside it. Then drive per-element transforms by **section scroll progress** in a controller hook (`useScrollController`), seeding start values from `animation-init-styles.json` and interpolating via the `bundles/*.js` scrollYProgress→transform mapping. Do NOT hardcode an arbitrary `min-height` with an empty runway — `forced-state-class` blocks it and the crop still fails because there is no scroll distance to drive the content. Do NOT force `is-active`/`opacity:1`/`transform:none` globally. See SKILL.md Step 7 architecture → "Scroll-scrub / pin sections — RUNWAY HEIGHT + progress-driven content". Inputs: `transition-coverage.json`, `animation-init-styles.json`, `bundles/*.js`, measured ref section heights. |
| **Quirks-mode (`BackCompat`) document breaks section-compare scroller auto-detect → all crops capture black** | The generated `impl/index.html` ships without a leading `<!doctype html>` (or has whitespace/BOM/markup before it), so Chrome renders in `BackCompat` quirks mode. In quirks mode `document.documentElement.clientHeight` reports the **full document height** instead of the viewport, so section-compare's scroller auto-detect cannot identify the document scroller and instead picks a too-generic tall `div` (e.g. a hidden FAQ panel) as the scroller. Forced-scroll then targets the wrong element → `actualY=0`, bottom-anchored or all-black crops, `timeout`/`black-crop` failures across most sections. Measured on a cold clone of a Lenis + scroll-scrub site: this single missing doctype stalled the entire verify path. **Detection:** `(()=>document.compatMode)()` returns `"BackCompat"` (standards mode returns `"CSS1Compat"`); section logs show the scroller resolved to a content `div` rather than `__document__`; crops are uniformly black. | Emit `<!doctype html>` as the **very first bytes** of `impl/index.html` (Vite's default template already does — do not prepend anything before it, do not save a captured `live.html`/`original.html` as the entry). Confirm `document.compatMode === "CSS1Compat"` before running section-compare. Standards mode restores `documentElement.clientHeight` to the viewport so the document scroller is detected correctly. |
| **Plain text wrapped in JSX expression containers (`{"..."}`) defeats text-fidelity source scan → false missing-text failures** | The generator emits visible copy as `<p>{"Long sentence, with commas\nand a line break"}</p>` instead of a literal text child. `text-fidelity-check` scans the JSX source with a regex over `{"..."}` literals; long sentences, embedded newlines, or a space-before-`>` / newline-before-closing-tag truncate the match, and lines containing `//` (e.g. a URL) are nuked by its comment strip — so genuinely-present copy reads as missing. This is a **source-scan fidelity** problem, not a runtime one. | Emit visible copy as **literal JSX text children** (`<p>Long sentence, with commas and a line break.</p>`), not `{"..."}` expression containers. Where a `<br>`/line break is required, split into sibling text + `<br/>` preserving inter-element whitespace **inside** the following span (the transpiler drops inter-element whitespace, gluing `text<span>`). Reserve `{...}` only for genuine interpolation. See `responsive-detection.md` "Common pitfall — extracting body text" for the `childNodes` extraction that feeds this. |
| **Lenis / smooth-scroll reverts the capture script's forced `scrollTo` → `actualY=0`, wrong-frame crops** | `section_capture` forces the ref's exact `scrollY` onto the impl to crop the same scroll frame, but a Lenis/Framer smooth-scroll controller intercepts the native `window.scrollTo` and animates back toward its own target (often 0) during the `_finish_js`/settle step, so the recorded `actualY` collapses to 0 and every scrubbed section crops at the start frame. **Detection:** capture log shows `actualY=0` despite a non-zero `forced_scroll_y`; only above-the-fold sections match. | On the generation side, gate the smooth-scroll init so it does not fight programmatic scroll during capture: honor a `?capture=1` / `window.__UI_CLONE_CAPTURE__` flag (or `prefers-reduced-motion`) that disables Lenis `requestAnimationFrame` and lets native scroll stand. The capture harness (`ui_clone/section_capture.py`) sets this flag before forcing scroll; without a generator hook the section-compare scroll mapping is unreliable on Lenis sites (cross-impl scroll-mapping class). |
| **`position:fixed` timed-animation overlay (rotating-word nav, ticker, active-section indicator) false-fails otherwise-faithful sections** | A `position:fixed`/sticky overlay renders at the TOP of EVERY section crop during section-compare. Its animation/label phase (active-section label width 0→auto, rotating word, dot indicator) cannot be frame-matched between the ref and impl capture instants, so its phase mismatch adds AE to every section — failing sections whose own content is faithful. Measured on a cold clone: masking the fixed nav dropped a text-heavy section's AE/Mpx ~12471→7679 (−38%); `sections/mask-coverage.json` read 0.0 for all sections because the nav was never declared dynamic. **Detection:** view `sections/diff/*.png` — a constant difference strip at the top of multiple crops that tracks a fixed overlay, not the section body. | Declare the overlay selector as `dynamic: true` in `transition-spec.json` so section-compare's DYNAMIC_SELECTORS masks it (the mask augments from `dynamic:true` entries; default `EXCLUDE_DYNAMIC=1`). This is legitimate, NOT a cheat — `masked-region-static` still enforces the overlay's static computed styles under the mask. After declaring, confirm `sections/mask-coverage.json` shows non-zero coverage for the affected sections; if it stays 0.0 the selector didn't match (check the exact class token) or masking didn't engage. Masking the overlay is **necessary but may not be sufficient** — a residual per-section content/position delta (e.g. a few-px vertical offset) is a separate clone-side fix. **NEVER declare `body`, `html`, `#root`, `main`, or any full-page/section-wrapping selector as `dynamic:true`** — DYNAMIC_SELECTORS masks the matched element, so a page-level target blanks EVERY crop and section-compare reports all sections `blank`/`unmeasured` (false self-pass that hides real AE). The dynamic target must be the **narrow overlay element only** (e.g. `.nav-active-label`, `.scroll-progress`), never its scroll ancestor. After declaring, verify `mask-coverage.json` is non-zero **and** that non-overlay crops are still actually measured (not all `blank`). |
...
| All section-compare AE failures treated as real bugs | Hero parallax images can receive a fresh random scale/position on every load, so the ref and impl legitimately start at different pixels. Header theme classes can also settle on different frames. | Read the diff image first. When bundle evidence contains `Math.random()`, `gsap.utils.random()`, or a `random*Range` field, declare the narrow animated target `dynamic: true` in `transition-spec.json`; the spec gate rejects stochastic fields without that declaration. Never mask the hero/section wrapper. Confirm masked-region static and runtime checks still verify layout and motion. Fix deterministic header-state differences in implementation rather than masking them. |
...
| Impl section count > ref (`structure.json` / `section-map.json` capture-state mismatch → spurious extra sections) | `structure.json` and `section-map.json` are captured at potentially DIFFERENT scroll/render states, so `structure.json` can carry `<section>` descendants (lazy/off-screen/collapsed content — e.g. a scroll-revealed category-card group `<section class=...sections_section>` nested inside a wrapper) that `section-map.json` never enumerated. The transpiler renders those nested `<section>` tags verbatim, and `section-compare` enumerates impl sections by `tag=section` — so each extra nested section becomes a spurious landmark, inflating the impl count vs the ref (measured: 18 impl vs 14 ref) and producing `EXTRA_IN_IMPL` / "extra impl section absent from ref" / index-shift duplicate findings. **Detection:** impl section count != `len(section-map.json)`, with EXTRA findings on `*__sections_text` / `*__sections_section`-style classes — a section-count/landmark problem, NOT content/styling. | **Auto-handled by the transpiler** (`scaffold-to-jsx.sh`): a nested `<section>` descendant whose class is not a `section-map` entry is demoted to `<div>` (content preserved; the claimed section ROOT and any genuinely-claimed nested section keep their `<section>` tag). So impl section count == `section-map` by construction. Do NOT hand-edit the generated component or attribute this to `html2/` enrichment — the transpiler reads `structure.json` + `section-map.json` only. The deeper data issue (two captures at different render states) is made harmless here; if the lazy sections must become REAL enumerated sections, recapture `section-map.json` at the same render state (extraction pipeline), not the transpiler. |
...
## Transpiler-baked px values — sizing-expressions consumption is MANDATORY
...
Rules (these are generation obligations, not suggestions):

1. **If `responsive/sizing-expressions.json` exists, every width / height /
   padding / font-size the scaffold baked as px MUST be re-resolved through
   it** — `vw` kind → viewport units, `calc`/`linear` → the `calc()`
   expression, `breakpoint-jump` → responsive prefixes, `fixed-px` → px is
   correct. Shipping the baked px for a selector that sizing-expressions
   classifies as non-fixed is a generation defect, not a style choice.
2. **Animated/scrubbed tracks must not keep baked values for the animated
   property.** When a runtime controller (scrub, spring, reveal) owns a
   property, clear the baked inline value at mount and let the controller
   drive it from its extracted expression — a baked px freezes the capture
   instant and diverges at every other scroll position.
3. **Never bake-then-tune.** If a baked px looks wrong at another viewport,
   the fix is the extracted expression (sizing-expressions, ref CSS grep),
   never a hand-tuned second px.

## Asset/SVG text forgery is PROHIBITED (svg-provenance)

Visible text must come from real DOM text nodes, verbatim-extracted SVG
`outerHTML` (`inline-svgs.json`), or the ref's own downloaded assets. The
following are anti-cheat violations, not approximations:

- Re-drawing a logo / label / wordmark as hand-authored SVG paths or `<text>`
  elements because the original asset was missing — recover the asset
  (`resource-mirror.sh`, `asset-download.sh`) instead.
- Editing text inside a downloaded SVG/asset (changing a label, swapping a
  brand string) to better match a screenshot.
- Rendering text into a `<canvas>` / generated image to imitate an asset the
  ref ships as a file.
- Substituting a look-alike asset and keeping the ref's filename so
  provenance scans match by name.

The `svg-provenance` gate verifies impl SVGs trace back to extracted ref
artifacts (`inline-svgs.json`, downloaded files) — fabricated geometry or
text fails provenance even when pixels look close. If an asset is genuinely
unrecoverable (404 on every mirror attempt logged in `download-log.json`),
declare it in `asset-substitution.json`; never forge it silently.

## Always-mount animated elements

Never conditionally MOUNT animated content (`{inView && <H2 .../>}`):
