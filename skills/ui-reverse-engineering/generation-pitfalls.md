...
| Unexpected space below the last card | Missing content, wrong sizing rule, or incorrect pin release | Compare source content, responsive sizing, and matched pin states; restore the observed rule instead of inventing a spacing allowance |
...
| Sticky track releases too early or a nested wrapper collapses | Wrong containing block, lost source sizing, or duplicated wrapper constraints | Restore the original track/inner ownership and authored responsive sizing. Apply track distance only to its owning element; verify pin entry/release instead of copying a height floor onto every descendant. |
| Scroll-linked section collapses or lower sections shift | Lost containing block, content, authored sizing, or actual pin/runway mechanism | Recover the source mechanism before changing height. Scroll-linked transforms alone do not prove pinning. Preserve responsive sizing and controller offsets; verify section placement at matched start/mid/end states. See `pipeline-execution.md` → Step 7 architecture. |
