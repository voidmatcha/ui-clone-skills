...
description: >-
  Clone or replicate a live website URL as React + Tailwind. Triggers on
  "clone <URL>", "rebuild this in react", "reverse-engineer this layout",
  "extract the animation from <URL>". Key signal — user has a **reference
  URL**, not a prompt/screenshot/Figma. Outputs React components with
  extracted values (DOM, computed styles, JS bundle params for GSAP/
  Framer/Lenis). NOT for general CSS help or screenshot-to-code.
metadata:
  filePattern:
...
- `impl-scope.json` (`impl-scope-check.sh`) — iteration only writes
  `scratch/loop-<N>/impl/**`; plugin tooling edits (skills/, scripts/,
