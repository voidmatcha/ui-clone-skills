...
| | 5d | `bundle-map.json`, `transition-spec.json` (DRAFT), `external-sdks.json`. Run `bash "$PLUGIN_ROOT/skills/visual-debug/scripts/verification-plan.sh" "$(pwd)/tmp/ref/<component>"` before the final spec inventory pass, map every true signal to evidence-backed `transitions[]` or a structured `skipped[]`, then run the plan script again after editing the spec → `verification-plan.json` (universal `hydration-check` row + signal-derived rows for scroll-scrub / IO-reveal / hover / paid-font sites). A plan signal is a runtime-check dispatch hint, not transition proof; boolean CSS reveal detection from `structure.json` + captured CSS must never fabricate a spec entry. ⛔ Gate: `spec` — refuses to pass until `verification-plan.json` exists; downstream `post-implement` enforces each declared check. **The Phase-2 driver auto-mints a placeholder `transition-spec.json` (`source: ui_clone.extraction_artifacts`, `placeholder: true`) — that is a draft floor, NOT your spec; the gate hard-fails it on motion sites. "Under-populated" is numeric: every `bundle-extraction.json` motion construction site (framerMotion/gsap/lenis/animeJs/webflowIX2), every `interactions-detected.json` row, and every true `verification-plan.json` signal class must map to ≥1 `transitions[]` entry or a `skipped[]` reason — `gate spec`'s spec-inventory-coverage enforces this. `bundle-extraction.json` is now produced deterministically by the Phase-2 driver (`scripts/extract/bundle-extraction.sh`, no subagent) and already holds the Lenis/GSAP/Framer/Anime/Webflow-IX2 construction sites with parameters — do NOT dispatch a subagent to re-derive those. Script-first, dispatch-on-gap: dispatch the host-neutral `bundle-analyzer` subagent role ONLY for the gaps the parser flags — every entry in `bundle-extraction.json` `unresolved[]` (Swiper/Splide and other carousel/slider configs the regex parser cannot parse), plus Lottie/bodymovin, custom scroll/anim, symbolic transform mappings, and multi-file state machines — or when spec-inventory-coverage is still short after the deterministic params are mapped to `transitions[]`. Prompt: "Read tmp/ref/<component>/bundles/*.js + bundle-map.json + the `unresolved[]` gaps in bundle-extraction.json, extract the flagged construction sites with parameters and MERGE into tmp/ref/<component>/bundle-extraction.json (do not overwrite the deterministic extractions)." Exit condition: `python -m ui_clone.gate <ref-dir> spec` passes spec-inventory-coverage — not merely "the artifact exists".** |
...
| | 6 | `animation-detection.md`. ALL browser-state captures run through live `agent-browser` sessions: A (idle/splash), B (scroll), C (hover/per-element), C-click (`capture-click.sh` when click candidates exist). The capture post-pass writes `state-structure-spec.json` (motion refinement, runs AFTER the base scaffold by default per `animation-detection.md` "Generate-first / time-box discipline"). Run A/B/C before generation ONLY when a cheap pre-Step-6 signal — `canvas-webgl-detection.json` hasCanvas, `dom-state-diff.json` preloader/class-flip, `scroll-engine.json` custom engine, or `interactions-detected.json` structure-altering interaction — flags structure-load-bearing motion; otherwise reach `generation-plan.json` + scaffold first, then run A/B/C and read `state-structure-spec.md` to refine. Canvas/WebGL → `canvas-webgl-extraction.md`. |
...
- **Scroll-scrub / pin sections — RUNWAY HEIGHT + progress-driven content (the two non-negotiables):** any section classified as scroll-scrub or sticky-pin — i.e. `transition-coverage.json` marks it scroll-scrub (not IO-reveal), OR `animation-init-styles.json` gives its children non-trivial start transforms/opacity, OR a `bundles/*.js` scroll-progress driver (Lenis `scrollYProgress` / GSAP `ScrollTrigger` scrub) maps section progress → transforms — has its height supplied by a **pin + runway**, not by its content. Two obligations:
  1. **Reserve the OUTER track (runway) to MATCH the ref section height.** The ref section has NO inline/CSS height — its height *is* the scroll distance of the scrub. Render an outer track div whose height equals the measured ref section height (from `ref-scroll-positions.json` / the section-height table you measured), with the visible content as a `position: sticky; top: 0; height: 100vh` inner that pins inside it. Do NOT let the section collapse to content height (it renders hundreds of px short). Do NOT instead hardcode an arbitrary `min-height` over an empty runway — that is blocked by `forced-state-class` AND still fails the crop because there is no scroll distance to drive the content. The runway IS the height; the sticky inner IS the visible pin.
  2. **Drive per-element transforms by SECTION SCROLL PROGRESS, not a static render.** Compute `progress = (scrollY − sectionTop) / (sectionHeight − viewportH)` (or subscribe to the ref library's `scrollYProgress` for that section) in a controller hook (e.g. `useScrollController`), seed each element's start value from `animation-init-styles.json`, and interpolate toward the end value using the `bundles/*.js` scrollYProgress→transform mapping. This is what makes read-along text dim/highlight, per-char disintegration, opacity/scale reveals, horizontal `translateX` marquees, and slide-up card stacks land at the correct mid-animation state when section-compare scrolls to a ref position. A static render freezes every such element at its init state and fails AE by 10–700×.
  Inputs that drive this, per section: `transition-coverage.json` (scroll-scrub vs IO-reveal classification), `animation-init-styles.json` (start states), `bundles/*.js` (scrollYProgress→transform mapping), and the measured ref section heights (the runway lengths). When the bundle mapping is unclear, dispatch the host-neutral `source-forensics` / `bundle-analyzer` role on `bundles/*.js` before generating — do not guess the curve.
