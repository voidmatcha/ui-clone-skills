...
| `--selector` screenshot times out | `scrollTo` the element's top first (it may be below the fold), then a plain viewport `agent-browser --session <s> screenshot` (no selector, no `--full`) + sips crop. |
