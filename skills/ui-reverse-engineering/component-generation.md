...
verification scopes. Preserve the downloaded local stylesheet cascade, including
media/container queries, original breakpoints, `%`/viewport units, `clamp()`/`calc()`,
Grid/Flex reflow, max-width constraints, and source visibility/order variants.
Do not discard inactive rules or hidden responsive branches because the representative
capture is desktop. Keep container-query ancestors and responsive asset choices.

Use authored CSS and `responsive/sizing-expressions.json` to resolve layout values.
Do not leave computed capture-time widths, heights, font sizes, or transforms inline
when they override the source's responsive rules. Preserve genuinely authored fixed
sizes; do not invent fluid formulas or replace every pixel value mechanically.

For replaced elements (`img`, `video`, `canvas`, `iframe`), preserve authored
inline sizing and fit rules as well as stylesheet rules. In particular, a source
`height: 100%; object-fit: contain` must not become `height: auto` or inherit a
conflicting `cover` rule. Intrinsic asset dimensions and captured box dimensions
are not interchangeable. If the capture retained only computed values, recover
the authored declarations from the live reference before choosing a sizing rule.

Section components must retain their shared layout ancestors. An absolute
background sibling and the foreground sections it covers belong inside the same
positioned ancestor; emitting the background into an empty standalone wrapper
changes its containing block and can make it disappear. Preserve those ancestor
boundaries when composing the app, including stacking and clipping contexts.
Text-node presence and equal section heights cannot validate these paint layers.

Before detailed desktop comparison, resize within the inferred desktop band and
check live geometry/overflow. Investigate the first broken constraint before starting
another expensive image sweep. Mobile-specific behavior and detailed mobile parity
may follow desktop completion, but responsive source structure must already exist.
Unobserved mobile structure is an evidence gap, never a reason to invent a layout or
claim responsive completion. This policy is implementation guidance, not gate evidence.
...
4. **Group subtrees into Section components** using `sections[]` metadata (`top`, `height`, `class`). Each section becomes one component under `impl/src/components/<Name>.tsx`. Component count MUST match `sections.length`.
   ...
   **Section sizing follows source rules.** Use measured section boxes to detect
   missing content, collapsed ownership, or incorrect layout, not as universal
   pixel-height floors. Preserve authored height/min-height, padding, aspect
   ratio, font metrics, containing blocks, and responsive expressions. A captured
   computed height is an observation at one viewport/state; it does not establish
   a fixed CSS rule. Do not add a spacer or `min-height` merely to make section or
   document totals match. Recover the source rule or pin mechanism before changing
   sizing; verify both content placement and the affected scroll states afterward.
...
6. Map each trigger to an observable mechanism before coding:
   ...
   - `scroll-state-machine` (raw `scrollY` pixel layout state): distinguish the source spec from the generated plan. `transition-spec.json` `animation.channels[]` uses `inputRange`/`outputRange`; by implementation time, `generation-plan.json` → `scrollStateMachine.sites[]` carries `inputDomain: "scroll-y-px"` on the site and `transforms[]` entries such as `property: "top"`, `unit: "px"`, `input: "[0,100]"`, and `output: "[56,20]"` (currently JSON-encoded numeric range strings). Treat those numbers as bundle-literal document pixels. Do NOT divide by capture `maxScroll`, do NOT convert them to `[0,1]` progress, and do NOT route them through the wrapper-based `<ScrollScrub>`. Emit `ScrollLinkedStyleDriver` or direct selector-scoped style updates against the real fixed/sticky target element. If the reference has separate desktop and mobile nav selectors, wire each breakpoint-specific selector/channel independently. If no evidence-backed spec entry exists, leave the channel absent instead of inventing nav motion.
