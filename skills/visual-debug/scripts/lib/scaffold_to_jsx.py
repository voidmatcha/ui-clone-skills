...
# D-family (ebpb loop-2): extract-dom stores el.tagName.toLowerCase(), but
...
def render(node, indent=2, hover_rules=None):
    ...
    # by half its own size (loop-129 Fix 68 regression).
    _captured_wh = {}
    ...
    attr_map = {
        ...
        "title": "title",
        "role": "role",
        # A-family (ebpb loop-1): dropping iframe `allow` made Chrome's
    ...
    if tag in SVG_TAGS or node.get("svg"):
        ...
        for src_key in list(SVG_ATTR_RENAMES.keys()) + list(SVG_PASSTHROUGH_ATTRS):
            ...
            if "url(" in v:
                ...
                v = rewrite_asset_url(v)
            attr_emit[jsx_key] = v
        # E-family (ebpb loop-1 tagline path-d class): extract-dom captures
    ...
    if tag == "video":
        ...
        if _vp and _vp.get("autoplay"):
            ...
            # realfood loop-145 muted-attr bug), so a visible hero/background
