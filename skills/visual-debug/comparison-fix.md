...
| **E. Full-page / resized-viewport capture** (a screenshot on either side was taken with `screenshot --full`/`-f`, or after `set viewport <w> <documentHeight>` / `<sectionHeight>`, instead of a fixed-size viewport shot) — cheapest check, try this first | Re-open at 1440×900, `scrollTo(<section top>)`, viewport `screenshot`. If the content now appears (or matches), the capture mode was wrong, not the impl. | Never use `--full`/`-f` or a document/section-height viewport on a pinned (`position:sticky`, GSAP `ScrollTrigger`) or scroll-driven section — expanding the viewport re-runs that layout at the new size and the section paints blank or mid-transform. An IntersectionObserver-revealed section is a different risk under `--full`: everything intersects at once, so its reveal fires (possibly mid-animation) rather than staying blank — still not valid evidence, just a different wrong state. Recapture with `batch-scroll.sh` / `section-compare.sh` / a `scrollTo → wait → viewport screenshot` loop instead. |

**Decision rule:** if any one of A–E explains the diff, fix the harness and re-run before reading the diff image. Reading a diff image first (or worse, editing impl CSS) wastes tokens and can introduce real bugs while chasing an artifact.

If A–E all check out clean and AE is still large → it's a real mismatch; proceed to `auto-diagnose.sh` and the rest of the fix protocol below.
