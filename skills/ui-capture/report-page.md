...
**Producing `fullpage.png`:** `static/ref/section-{0..4}.png` (from this driver's Phase 1 capture) are 5 EVENLY-SPACED SAMPLES at `Y = scrollHeight * i/5` (`scripts/extract/capture.sh:270-276`) — NOT contiguous tiles. Appending them with ImageMagick produces gaps or overlaps against `regions.json`'s coordinates unless the page height happens to be an exact multiple of the viewport height. Build `fullpage.png` from a real contiguous scroll instead: loop `k = 0, 1, 2, ...` while `k*900 < scrollHeight`, `scrollTo(k*900)` → `wait 800` → viewport `screenshot`; crop the LAST tile to `scrollHeight - k*900` (scroll clamps at `document.documentElement.scrollHeight - innerHeight`, so the final shot overshoots) before `magick tile-0.png tile-1.png ... -append fullpage.png`. **Never** use `agent-browser screenshot --full`/`-f` to make it directly: on a pinned (`position:sticky`, GSAP `ScrollTrigger`) or scroll-driven site, `--full` expands the layout viewport to document height in a single capture, which re-runs that layout at the new size and renders the pinned/scroll-driven region blank or mid-transform — corrupting exactly the coordinates `regions.json` expects the overlays to line up against. The tile loop above has the same corruption risk if `900` isn't the viewport height actually used — keep them in sync.
